...
// Sorry about the mess. There is little time for me to do a real clean-up for code.
...
const validateRequest = async (req, res, next) => {
	...
	if (
		studyPlan &&
		(await Promise.all(studyPlan.map(code => coursesDAO.getCourseByCode(code))).then(
			rets => rets.some(ret => !ret)
		))
	)
		return res.status(422).send({ error: "Study Plan contains abnormal chunks" });
	...
	if (studyPlan && studyPlan.length > 0 && isFullTime) {
		...
		studyPlanCourseList.forEach(course => {
			...
			if (
				...
				studyPlan.some(code => course.incompatible_list.split(" ").includes(code))
			)
				return res.status(422).send({ error: "List contains conflict pairs" });
			if (
		...
		if (currCredits > (isFullTime ? 80 : 40) || currCredits < (isFullTime ? 60 : 20))
			return res.status(422).send({
...
app.get("/api/courses", async (req, res) => {
	return await coursesDAO.getAllCourses().then(
		courses => {
			return courses ? res.status(200).json(courses) : res.status(404).end();
		},
		err => {
...
app.get("/api/studyPlans/:matricola", isLoggedIn, validateRequest, async (req, res) => {
	return await studyPlanDAO.getStudyPlanById(req.params.matricola).then(
		studyPlan => {
			return studyPlan ? res.status(200).json(studyPlan) : res.status(404).end();
		},
		err => {
			return res.status(500).send(err);
		}
...
app.post("/api/studyPlans/:matricola", isLoggedIn, validateRequest, async (req, res) => {
	return await studyPlanDAO
		.addStudyPlan(req.params.matricola, req.body.isFullTime, req.body.courses)
		.then(
			() => {
...
app.put("/api/studyPlans/:matricola", isLoggedIn, validateRequest, async (req, res) => {
	return await studyPlanDAO
		.updateStudyPlan(req.params.matricola, req.body.isFullTime, req.body.courses)
		.then(
			ret => {
...
app.delete(
	...
	async (req, res) => {
		return await studyPlanDAO.deleteStudyPlan(req.params.matricola).then(
			studyPlan => {
				return studyPlan ? res.status(204).end() : res.status(404).end();
			},
			err => {
				return res.status(500).send(err);
			}
...
// APIs for booking/un-booking single course.

app.put("/api/courses/book/:code", isLoggedIn, validateRequest, async (req, res) => {
...
app.put("/api/courses/unBook/:code", isLoggedIn, validateRequest, async (req, res) => {
